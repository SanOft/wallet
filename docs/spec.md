...
- **FR-2.6** **Rotation:** on every refresh, the old token is invalidated and a new one is issued. Logout revokes the refresh token family but, for the same self-contained-JWT reason as FR-2.7's bound, leaves the current access token valid on read routes for up to its FR-2.4 lifetime (15 minutes) after logout — by design, not an oversight.
...
- **FR-2.8** **Step-up:** any single transfer above 1,000,000 UZS requires re-entering the password on the confirmation screen. Enforced in `TransferService`, **after** the idempotency replay lookup rather than before it: a refusal writes no record, so ordering it first protects nothing — while it would force the offline outbox (FR-8.3) to re-confirm money that has already moved, which it cannot do, because a queued transfer must never carry a password into IndexedDB. The confirmation itself is `AuthService`'s, shared with login and with FR-1.6's PIN change: one credential, one counter, one lock. FR-2.3's backoff is therefore read *before* the password is verified here too — three wrong confirmations and the fourth answers `AUTH_LOCKED` (429, `Retry-After`) instead of `STEP_UP_FAILED`, and the sign-in for that number is locked with it. Recording the failures without ever reading the count, which is what this did first, slowed the next login and left the transfer endpoint an unmetered password oracle for whoever held the session.
...
**`AUTH_LOCKED` is not only a sign-in answer.** `POST /api/transfers` (above the FR-2.8 threshold) and `PUT /api/me/pin` verify the account password through the same operation `POST /api/auth/login` uses, so either may answer `429 AUTH_LOCKED` with a `Retry-After` header once FR-2.3's backoff is active for that number — and failures spent on one of the three count against all three. Both are additionally metered per address, separately from the sign-in budget so that ordinary refused transfers do not spend somebody's allowance to sign in.
...
| `AUTH_LOCKED`              | 429  | FR-2.3's backoff is active for the number — on login, and on the password confirmation `/transfers` and `/me/pin` ask for (`Retry-After` header) | "Too many attempts. Try again in X minutes."                                                                   |
...
2. **Amount:** large numeric input; the current balance, the per-operation cap and the remaining daily allowance below it; exceeding any of the three produces a real-time error (client-side via the same shared Zod, without waiting for `LIMIT_EXCEEDED`). The allowance is served by `GET /api/accounts`, computed by the same function that would refuse the transfer, so the figure on screen cannot disagree with the check inside the transaction; it is the rolling 24-hour window of FR-6.1, not a calendar day. While it is unknown the row shows an em dash and nothing is blocked — the server holds the real gate, and a request in flight must not refuse a legitimate transfer. (Until P-32 closed, this was a deliberate omission: the only client-side route to the figure was summing a paged history, which is right until somebody exceeds one page in a day and silently understates their spending after that.)
...
| **R**epudiation            | "I never sent this money"                                                        | Every operation: ledger pair + `channel` + logs keyed on a server-minted `traceId` the caller cannot set (P-24) + the transaction ID shown to the user | FR-5.3, NFR-5            |
...
|                            | A stolen session reads the victim's payment history, which now carries recipient **phone numbers**, not only masked names (P-36)                | Accepted, and bounded: the number is returned only on an **outgoing** transfer — a number the user typed themselves — and is `null` on incoming, so a session never discloses who paid *the user*. No new enumeration: it returns nothing an attacker with the session could not already reach.        | FR-5.3, P-36             |
