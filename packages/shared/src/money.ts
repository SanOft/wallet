...
export const CURRENCIES = {
  UZS: { code: 'UZS', symbol: "so'm", exponent: 2, displayDecimals: 0, symbolPosition: 'suffix', groupSeparator: ' ', decimalSeparator: ',' },
  USD: { code: 'USD', symbol: '$',    exponent: 2, displayDecimals: 2, symbolPosition: 'prefix', groupSeparator: ',', decimalSeparator: '.' },
  EUR: { code: 'EUR', symbol: '€',    exponent: 2, displayDecimals: 2, symbolPosition: 'suffix', groupSeparator: '.', decimalSeparator: ',' },
...
export const moneySchema = z.string().regex(CANONICAL_MINOR_RE, {error:'money.invalid_format'}).transform((s) => BigInt(s))

export type Money = z.infer<typeof moneySchema>

export function createTransferAmountSchema(currency: SupportedCurrency){
    const {min, max, step} = TRANSFER_LIMITS[currency]
    return moneySchema.refine((v)=> v >= min, {error:'money.below_minimum'}).refine((v)=> v <= max, {error: 'money.above_maximum'}).refine((v)=> v % step === 0n, {error:'money.invalid_step'}) 
}
...
if(c.displayDecimals > 0){
    body += c.decimalSeparator + fractional.toString().padStart(c.exponent, '0').slice(0, c.displayDecimals)
}
