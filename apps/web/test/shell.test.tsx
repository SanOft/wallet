...
describe("the application shell", () => {
  ...
  it("gives the navigation a name, because a page can have several", () => {
    render(<App />)
    expect(screen.getByRole("navigation", { name: /navigatsiya/i })).toBeInTheDocument()
  })

  it("offers exactly the three tabs §13.3 specifies", () => {
    render(<App />)
  ...
  it("marks the current tab for a screen reader, not only in colour", () => {
  ...
  it("sizes every tab to the minimum touch target (NFR-3)", () => {
    render(<App />)
  ...
  it("carries a skip link, and keeps it out of the way until focused", () => {
    render(<App />)
  ...
  it("answers an unknown path with a way out rather than a dead end (§13.1)", () => {
